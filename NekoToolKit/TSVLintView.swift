...
struct TSVLintView: View {
    ...
    private func lintTSV() {
        let lines = inputText.components(separatedBy: .newlines)
        guard let firstValid = lines.first(where: { !$0.trimmingCharacters(in: .whitespaces).isEmpty }) else {
            resultText = "⚠️ 無資料可檢查"
            resultColor = .red
            errorLines = []
            return
        ...
        let expectedCount = firstValid.components(separatedBy: "\t").count
        ...
        for (index, line) in lines.enumerated() {
            ...
            let actualCount = line.components(separatedBy: "\t").count
        ...
        let validLineCount = lines.filter { !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }.count

        if invalidLineNumbers.isEmpty {
            resultText = "✅ 格式正確，共 \(validLineCount) 行，欄位數一致（\(expectedCount) 欄）。"
            resultColor = .green
        } else {
            resultText = """
❌ 發現欄位數不一致：
預期欄數：\(expectedCount)
異常行號：\(invalidLineNumbers.map(String.init).joined(separator: ", "))
"""
            resultColor = .red
        }
    }
